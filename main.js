...
const openW = (lat, lon) => {
  const fetching = fetch(
    ...
    .then((w) => {
      ...
      pToday.textContent = w.list[0].main.temp + '°C';
      pClouds.textContent = w.list[0].clouds.all + '%';
      pHumidity.textContent = w.list[0].main.humidity + '%';
        ...
        if (wet.childNodes.length < 5) {
          ...
          el.list.forEach((el) => {
            ...
            if (hours.includes(hour)) {
              ...
              switch (hour) {
                case '06:00:00':
                  console.log('hit gold baby');
                  temper.push(el.main.temp);
                  console.log(temper);

                  lowestTempMin2 = Math.min(...temper) - 2;
    ...
    .then((week) => {
      ...
      gradient.addColorStop(0, 'rgba(255, 51, 48,0.6');
      gradient.addColorStop(0.7, 'rgba(255, 166, 35,0.4');
      gradient.addColorStop(1, 'rgba(56, 159, 252,0.2');
      ...
      const config = {
        ...
        options: {
          ...
          scales: {
            ...
            y: {
              min: lowestTempMin2, ///minimum for y axis
              ticks: {
      ...
      localStorage.setItem(`id-${cityname}`, wet.innerHTML);
    })
    .catch((err) => console.log('err', err));
};
// openW();

wet.innerHTML = localStorage.getItem('id-brussels');
